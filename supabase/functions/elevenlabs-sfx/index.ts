...
serve(async (req) => {
  ...
  try {
    ...
    // Validate duration (0.5 to 22 seconds)
    const durationSeconds = Math.min(Math.max(Number(duration) || 5, 0.5), 22);
    ...
    console.log('Generating sound effect with ElevenLabs for prompt:', prompt.substring(0, 50) + '...');
