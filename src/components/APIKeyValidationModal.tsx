...
export const APIKeyValidationModal = forwardRef<HTMLDivElement, APIKeyValidationModalProps>(
  function APIKeyValidationModal({ isOpen, onClose, apiKeyName, onSuccess }, ref) {
    ...
    if (validationState === "success" || validationState === "error") {
      return (
        <Dialog open={isOpen} onOpenChange={handleValidationClose}>
          <DialogContent ref={ref} className="sm:max-w-md panel-3d border-2 border-[hsl(var(--primary))]/30">
    ...
    return (
      <Dialog open={isOpen} onOpenChange={(open) => !open && handleValidationClose()}>
        <DialogContent ref={ref} className="sm:max-w-lg panel-3d border-2 border-[hsl(var(--primary))]/30">
          <DialogHeader>
